...
function gh() {
  ...
  return { owner: d.owner || "", repo: d.repo || "", token: d.token || "" };
}
KHOOD.ghReady = function () {
  const g = gh();
  if (!g.owner || !g.repo || !g.token) return false;
  if (g.token.indexOf("حط_") === 0 || g.owner.indexOf("اكتب_") === 0) return false;
...
KHOOD.fileGet = async function (path) {
  if (!KHOOD.ghReady()) return { ok: false, data: null, sha: null, status: 0 };
  try {
    ...
    if (!r.ok) return { ok: false, data: null, sha: null, status: r.status };
