(function () {
  ...
  function people() {
    ...
    KHOOD.CHARS.filter(function (c) { return c.id !== me; }).forEach(function (c) {
      ...
      row.innerHTML = "<span class='relative'><img src='" + c.img + "' class='w-12 h-12 rounded-full object-cover'><span class='absolute bottom-0 left-0 w-3 h-3 rounded-full " + (on ? "bg-green-400" : "bg-gray-500") + "'></span></span><div><b class='block'>" + c.name + "</b><small class='text-gray-400'>" + (on ? "متصل" : "أونلاين لما يفتح") + "</small></div>";
  ...
  function threadList() {
    return (hist[withId] || []).slice().sort(function (a, b) { return (a.at || 0) - (b.at || 0); });
  }
  function paintLog() {
    const log = el("log");
    if (!log) return;
    log.innerHTML = threadList().map(function (m) {
      ...
      return "<div class='" + side + "'>" + String(m.t || "").replace(/[<>]/g, "") + "</div>";
    }).join("");
  ...
  function openThread(id) {
    ...
    if (el("youSeen")) el("youSeen").textContent = group ? "جروب" : "الرسايل من المستودع";
    showThread(true);
    paintLog();
    pullThread(id);
  }
  ...
  function absorb(list, key) {
    ...
    (list || []).forEach(function (m) {
      const id = m.id || (m.u + ":" + m.at + ":" + (m.t || ""));
      if (seenIds[id]) return;
      seenIds[id] = 1;
      hist[key].push(m);
      n += 1;
      if (m.u && m.u !== me) notify(KHOOD.cname(m.u), m.t || "رسالة");
    });
  ...
  async function pullThread(id) {
    ...
    if (file.ok && file.data && file.data.msgs) absorb(file.data.msgs, id);
  }
  ...
  if (el("sendForm")) el("sendForm").onsubmit = function (e) {
    ...
    el("msgIn").value = "";
    const item = { id: mid(), u: me, t: text, at: Date.now() };
  ...
  if (el("recBtn")) el("recBtn").onclick = async function () {
    ...
    try {
      ...
      rec.onstop = function () {
        ...
        if (blob.size > 180000) { stat("الصوت طويل"); return; }
        const reader = new FileReader();
        reader.onload = function () {
          const item = { id: mid(), u: me, a: reader.result, at: Date.now() };
          if (!hist[withId]) hist[withId] = [];
