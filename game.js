(function () {
  ...
  const CHARS = [
    ...
    { id: "fady", name: "فادي", color: "#2ecc71", price: 250, perk: "hp", speed: 1, hp: 145, def: 1 },
    { id: "wageeh", name: "وجيه", color: "#f1c40f", price: 250, perk: "speed", speed: 1.2, hp: 100, def: 1 },
    { id: "koko", name: "كوكو", color: "#3498db", price: 250, perk: "coins", speed: 1, hp: 100, def: 1 },
    { id: "tony", name: "توني", color: "#9b59b6", price: 250, perk: "armor", speed: 1, hp: 110, def: 0.55 },
    { id: "sisi", name: "السيسي", color: "#223445", price: 250, perk: "boss", speed: 1.12, hp: 130, def: 0.8 }
  ];
  const SKINS = [
    { id: "none", name: "من غير إطار", ring: null, price: 0 },
    { id: "gold", name: "إطار دهبي أوي", ring: "#ffd24a", price: 80 },
    { id: "neon", name: "إطار لمبة أفراح", ring: "#19f0ff", price: 90 },
    { id: "fire", name: "إطار الفرن", ring: "#ff6a1a", price: 100 }
  ];
  const ASSISTS = [
    { id: "fesya", name: "السرعة الخارقة", desc: "تطير 6 ثواني بسرعة زيادة", price: 90 },
    { id: "batata", name: "درع البطاطا", desc: "صدمة واحدة تتاكل", price: 80 },
    { id: "shafat", name: "شفاط الفلوس", desc: "يسحب القروش 8 ثواني", price: 70 },
    { id: "nasb", name: "ماكينة النصب", desc: "القروش تتضاعف 10 ثواني", price: 100 },
    { id: "maka", name: "فرامل السيسي", desc: "تطبّل اللي وراك 7 ثواني", price: 85 },
    { id: "aseer", name: "علبة عصير مجروحة", desc: "ترجعلك 40 دم", price: 60 },
    { id: "khod", name: "خووود مسبّقة", desc: "المطارد يتلخبط ثانيتين", price: 100 },
    { id: "slip", name: "شبشب الجري", desc: "الحيطان تضربك أهدى في المرحلة دي", price: 75 },
    { id: "luck", name: "حظ أبو كلبشتين", desc: "يرمي قروش زيادة حواليك", price: 65 },
    { id: "3aks", name: "مخ الضد", desc: "ممكن يقلب التحكم عليك. دي المساعدة يا نجم", price: 95 },
    { id: "ghost", name: "اختفاء بالراحة", desc: "شبح 10 ثواني. مفيش حاجة تأثر فيك", price: 100 },
    { id: "3een", name: "عين صقر أبو نضارة", desc: "يبين المطارد لو بعيد", price: 55 }
  ];
  const JOKES = {
  ...
  function doDash() {
    ...
    if (save.dashes >= 20) unlockAch("dash", "صاحب السرعة");
  ...
  function mkOnline(host, code) {
    ...
    net.peer = new window.Peer(undefined, { config: { iceServers: [{ urls: "stun:stun.l.google.com:19302" }] } });
    net.peer.on("call", function (call) {
    ...
    net.peer.on("open", async function (id) {
      net.myId = id;
      if (host) {
        net.peeps[id] = { x: 80, y: 80, name: netName(), role: net.wantHunt ? "hunt" : "run", dead: false };
        try { if (window.KHOOD && KHOOD.put) await KHOOD.put("khoood-room-" + net.room.toLowerCase(), { host: id, at: Date.now() }); } catch (e) {}
        if (net.keep) clearInterval(net.keep);
        net.keep = setInterval(function () {
          if (window.KHOOD && KHOOD.put) KHOOD.put("khoood-room-" + net.room.toLowerCase(), { host: id, at: Date.now() });
        }, 4000);
        netStatus("الغرفة " + net.room + " — ابعت الكود لصحابك");
        netList();
      } else {
        let hostId = null;
        try { const info = window.KHOOD && KHOOD.get ? await KHOOD.get("khoood-room-" + net.room.toLowerCase()) : null; hostId = info && info.host; } catch (e) {}
        if (!hostId) { netStatus("الغرفة مش مفتوحة. استنى صاحبها ويعملها تاني"); return; }
        function tryJoin(left) {
          ...
          conn.on("error", function () {
            if (left > 0) { netStatus("بجرب أدخل تاني..."); setTimeout(function () { tryJoin(left - 1); }, 1200); }
            else netStatus("صاحب الغرفة قفل أو النت ضعيف");
          });
        }
        tryJoin(3);
      }
    ...
    net.peer.on("error", function () {
      netStatus("الاتصال وقع. اعمل الغرفة من جديد");
    });
  }
  ...
  if (ytPlay) ytPlay.onclick = function () { playRoomMedia(document.getElementById("ytIn").value, false); };
  if (ytStop) ytStop.onclick = function () { stopRoomMedia(false); };
  ...
  const rt=document.getElementById("onRetry");
  if (rt) rt.onclick = function () { const code=(document.getElementById("roomIn").value||"").trim(); if (code) mkOnline(!!net.host, code); };
  ...
  const chat = { peer: null, conns: {}, me: "", pick: "", locked: false, pass: "", hist: {}, friends: [], groups: [], seen: {}, outbox: [], with: null, media: null, call: null, muted: false };
  function b64(bytes) {
    let s = "";
  ...
  function paintWho() {
    ...
    castList().forEach(function (c) {
      ...
      if ((chat.pick || chat.me) === c.id) b.style.outline = "3px solid #25d366";
      if (chat.locked) b.disabled = c.id !== chat.me;
      b.onclick = function () {
        if (chat.locked && c.id !== chat.me) { chatStat("شخصيتك اتقفلت. مش هتتغير"); return; }
        chat.pick = c.id;
        paintWho();
      };
      box.appendChild(b);
  ...
  async function loadChat() {
    try {
      ...
      if (!blob) return { user: localStorage.getItem("khoood-chat-user") || "", friends: [], groups: [], hist: {}, seen: {}, outbox: [], locked: false, pass: "" };
      const plain = await decText(blob);
      return plain ? JSON.parse(plain) : { user: "", friends: [], groups: [], hist: {}, seen: {}, outbox: [], locked: false, pass: "" };
    } catch (e) { return { user: "", friends: [], groups: [], hist: {}, seen: {}, outbox: [], locked: false, pass: "" }; }
  }
  async function saveChat() {
    const data = { user: chat.me || "", friends: chat.friends || [], groups: chat.groups || [], hist: chat.hist || {}, seen: chat.seen || {}, outbox: chat.outbox || [], locked: !!chat.locked, pass: chat.pass || "" };
  ...
  function showList(on) {
    document.getElementById("waList").style.display = on ? "flex" : "none";
    document.getElementById("waThread").style.display = on ? "none" : "flex";
  }
  function chatPaint() {
    ...
    log.innerHTML = list.map(function (m) {
      const mine = m.u === chat.me;
      if (m.k === "inv") return "<div class='bubble " + (mine?"me":"them") + "'>دعوة لعب للغرفة " + m.room + "<br><button type='button' class='btn' data-join='" + m.room + "'>انضم دلوقت</button></div>";
      return "<div class='bubble " + (mine?"me":"them") + "'>" + String(m.t || "").replace(/[<>]/g, "") + "<small>" + faceName(m.u) + "</small></div>";
    }).join("");
    ...
    const face = document.getElementById("chatFace");
    if (who) who.textContent = chat.with ? faceName(chat.with) : "";
    if (seen) seen.textContent = chat.with && chat.with.charAt(0) !== "#" ? seenText(chat.with) : "جروب";
    if (face && chat.with && chat.with.charAt(0) !== "#") face.src = faceSrc(chat.with);
  }
  function paintPeople() {
    ...
    box.innerHTML = rows.join("") || "<div class='shop-item'>ضيف شخصية تبدأ تتكلم معاها</div>";
    box.querySelectorAll("[data-open]").forEach(function (b) {
      b.onclick = function () { openThread(b.getAttribute("data-open")); };
    });
  }
  function chatSendTo(name, obj, queued) {
    const raw = JSON.stringify(obj);
    const id = uidOf(name);
    const have = chat.conns[id];
    if (have && have.open) { try { have.send(raw); return true; } catch (e) {} }
    if (!queued && obj.t !== "seen") {
      chat.outbox = chat.outbox || [];
      chat.outbox.push({ to: name, payload: obj, at: Date.now() });
      saveChat();
      dropInBox(name, obj);
      chatStat("الرسالة اتحفظت. أول ما يفتح هتوصل");
    }
    if (!chat.peer) return false;
    try {
      const c = chat.peer.connect(id, { reliable: true });
      c.on("open", function () { chatHook(c, name); c.send(raw); flushOutbox(); });
      c.on("error", function () { chatStat(faceName(name) + " مش ظاهر دلوقت"); });
    } catch (e) { chatStat(faceName(name) + " مش ظاهر"); }
    return false;
  }
  ...
  async function hashPass(s) {
    const buf = await crypto.subtle.digest("SHA-256", new TextEncoder().encode("khoood-pass:" + s));
    return b64(buf);
  }
  function lockBox() {
    const box = document.getElementById("waLock");
    if (box) box.style.display = chat.locked ? "none" : "block";
  }
  async function unlockChar() {
    const pick = chat.pick || chat.me;
    const pass = (document.getElementById("chatPass").value || "").trim();
    if (!pick) { chatStat("اختار شخصية"); return; }
    if (pass.length < 3) { chatStat("كلمة السر قصيرة"); return; }
    const h = await hashPass(pick + ":" + pass);
    if (chat.locked && chat.me !== pick) { chatStat("شخصيتك اتقفلت"); return; }
    if (chat.pass && chat.pass !== h) { chatStat("كلمة السر غلط"); return; }
    chat.me = pick;
    chat.pass = h;
    chat.locked = true;
  ...
  function boxUrl(id) { return "https://kvs.ix.workers.dev/khoood-mbox-v1-" + uidOf(id); }
  async function readBox(id) {
    try {
      const r = await fetch(boxUrl(id), { cache: "no-store" });
      if (!r.ok) return [];
      const j = await r.json();
      return (j && j.msgs) ? j.msgs : [];
    } catch (e) { return []; }
  }
  async function writeBox(id, msgs) {
    try {
      await fetch(boxUrl(id), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ msgs: (msgs || []).slice(-80) })
      });
    } catch (e) {}
  }
  async function dropInBox(to, payload) {
    const msgs = await readBox(to);
    msgs.push({ id: Date.now() + "-" + Math.random().toString(36).slice(2, 7), payload: payload, at: Date.now() });
    await writeBox(to, msgs);
  }
  async function drainBox() {
    if (!chat.me) return;
    const msgs = await readBox(chat.me);
    if (!msgs.length) return;
    let n = 0;
    msgs.forEach(function (item) {
      const msg = item.payload || item;
      if (!msg || !msg.u) return;
      const key = msg.g ? ("#" + msg.g) : msg.u;
      if (!chat.hist[key]) chat.hist[key] = [];
      const exists = chat.hist[key].some(function (m) { return m.at === msg.at && m.t === msg.m && m.u === msg.u; });
      if (exists) return;
      if (chat.friends.indexOf(msg.u) < 0 && msg.u !== chat.me && msg.u && msg.u.charAt(0) !== "#") chat.friends.push(msg.u);
      if (msg.t === "inv") chat.hist[key].push({ k: "inv", u: msg.u, room: msg.room, at: msg.at || Date.now() });
      else if (msg.t === "c") chat.hist[key].push({ u: msg.u, t: msg.m, at: msg.at || Date.now() });
      n += 1;
    });
    await writeBox(chat.me, []);
    if (n) { await saveChat(); paintPeople(); chatPaint(); chatStat("وصلك " + n + " رسالة"); }
  }
  function flushOutbox() {
    (chat.outbox || []).slice().forEach(function (item) {
      const id = uidOf(item.to);
      const c = chat.conns[id];
      if (c && c.open) {
        try { c.send(JSON.stringify(item.payload)); chat.outbox = chat.outbox.filter(function (x) { return x !== item; }); saveChat(); } catch (e) {}
      } else chatSendTo(item.to, item.payload, true);
    });
  }
  function goOnlineUser() {
    if (!window.Peer) { chatStat("محتاج نت"); return; }
    if (!chat.locked || !chat.me) { chatStat("افتح الشخصية بكلمة السر الأول"); return; }
    document.getElementById("chatUser").value = chat.me;
    try { if (chat.peer) chat.peer.destroy(); } catch (e) {}
    chat.peer = new window.Peer(uidOf(chat.me));
    chat.peer.on("open", function () {
      chatStat("ظاهر كـ " + faceName(chat.me));
      (chat.friends || []).forEach(function (u) { chatSendTo(u, { t: "seen", u: chat.me, at: Date.now() }); });
      flushOutbox();
      drainBox();
      if (chat.timer) clearInterval(chat.timer);
      chat.timer = setInterval(function () { flushOutbox(); drainBox(); }, 7000);
    });
    chat.peer.on("connection", function (c) { chatHook(c); chatSendToPeerSeen(c); flushOutbox(); });
  ...
  function showCall(call, local) {
    ...
    document.getElementById("callName").textContent = chat.with ? faceName(chat.with) : "مكالمة";
    document.getElementById("vidMe").srcObject = local;
    call.on("stream", function (s) { document.getElementById("vidYou").srcObject = s; });
    call.on("close", hangCall);
  }
  function hangCall() {
    try { if (chat.call) chat.call.close(); } catch (e) {}
    try { if (chat.media) chat.media.getTracks().forEach(function (tr) { tr.stop(); }); } catch (e) {}
    chat.call = null; chat.media = null; chat.muted = false;
  ...
  document.getElementById("chatBtn").onclick = function () { location.href = "./messenger.html"; };
  const _oldChatBtn = async function () {
    ...
    chat.locked = !!saved.locked && !!chat.me;
    chat.pass = saved.pass || "";
    chat.outbox = saved.outbox || [];
    document.getElementById("chatUser").value = chat.me;
    lockBox();
    paintWho();
    paintPeople();
    showList(true);
    show("chat");
    if (chat.locked) goOnlineUser();
  };
  document.getElementById("chatBack").onclick = function () { show("menu"); };
  document.getElementById("chatThreadBack").onclick = function () { showList(true); };
  document.getElementById("chatOn").onclick = function () { unlockChar(); };
  document.getElementById("hangBtn").onclick = function () { hangCall(); };
  document.getElementById("callMute").onclick = function () {
    chat.muted = !chat.muted;
    if (chat.media) chat.media.getAudioTracks().forEach(function (tr) { tr.enabled = !chat.muted; });
    document.getElementById("callMute").textContent = chat.muted ? "افتح" : "كتم";
  };
  ...
  if (q.get("room")) {
    show("online");
    document.getElementById("roomIn").value = q.get("room");
    mkOnline(q.get("host") === "1", q.get("room"));
  } else show("menu");
})();
