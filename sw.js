...
self.addEventListener("periodicsync", function (event) {
  if (event.tag !== "khood-mail") return;
  event.waitUntil((async function () {
    try {
      const key = await caches.open("khood-watch").then(function (c) { return c.match("key"); }).then(function (r) { return r ? r.text() : ""; });
      if (!key) return;
      const res = await fetch("https://kvs.ix.workers.dev/" + key, { cache: "no-store" });
      const j = await res.json();
      if (j && j.msgs && j.msgs.length) {
        await self.registration.showNotification("رسالة جديدة", { body: "عندك رسائل في khood community", icon: "./icon-192.png", data: { url: "./messenger.html" } });
      }
    } catch (e) {}
  })());
});
